...
--   3. There are 182 permission keys and the list grows. They live in a
--      catalog TABLE, not a Postgres enum, so adding one is an INSERT rather
--      than an ALTER TYPE (which locks and cannot run in a transaction
...
comment on table public.permissions is
  'Catalog of PermissionKey from src/types/facility-staff.ts (182 keys). Seeded, not hand-written — see seed.sql.';
