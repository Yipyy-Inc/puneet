-- ============================================================================
-- A grooming service, add-on, station or room the facility made can be booked.
--
-- `create_booking` resolved every catalogue choice by `legacy_id` alone:
--
--   grooming_services   s.legacy_id = p_grooming->>'serviceId'
--   grooming_add_ons    a.legacy_id = requested
--   grooming_stations   st.legacy_id = p_grooming->>'stationId'
--   facility_rooms      r.legacy_id = p_boarding->>'roomId'
--
-- Legacy ids exist only on rows that arrived by seed or backfill. A service,
-- add-on, station or room created through the app has none — the routes
-- address such a row by its uuid ("the app id is legacy_id when present,
-- else the uuid", in every one of those routes) — so the facility could
-- create a service on its menu and never book it: "This facility has no
-- grooming service <uuid>."
--
-- Each lookup now accepts either id. Nothing that matched before stops
-- matching: a legacy id is never a uuid's text, and both are still scoped to
-- the booking's facility. The rest of the function is unchanged, word for
-- word, from 20260806560000 and the migrations that followed it.
-- ============================================================================
...
begin
  if has_function_privilege('anon',
       'public.create_booking(jsonb, uuid[], jsonb, jsonb)', 'execute') then
    raise exception 'anon can create a booking';
  end if;
end $verify$;
