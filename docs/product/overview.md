...
- **Billing & money.** Payments, payment settings/methods, deposits, store credit, gift cards, cash drawer. (Observed: `facility/dashboard/payments`, `facility/dashboard/billing/{cash-drawer,payment-settings}`, [src/types/payments.ts](../../src/types/payments.ts).) **Invoicing is not among them** — there is no `invoices` table, no API route, and nothing in `src/` that creates one. It was listed here on the strength of `/facility/dashboard/billing` showing five fixture invoices; that screen was removed on 2026-09-02.
