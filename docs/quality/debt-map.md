...
### 🟢 A refund made inside Clover reaches the booking — and three ways it silently did not (fixed 2026-08-26)

**Start here, because it is the part most likely to be rebuilt by mistake: this already existed.** A refund a manager issues in Clover's own dashboard has reached Yipyy since 8 August, by webhook (`api/webhooks/clover/route.ts`) with a sweep as the backup for the webhook that never came (`lib/clover/sweep.ts`, `deploy/clover-sweep.timer`, every **15 minutes** since the VPS move — Vercel Hobby's one-cron-a-day limit was the whole reason it used to take up to 24 hours). Both end in one gap-based reconciler, and the booking moves itself because `payments_move_the_booking` recomputes `amount_paid` from the ledger. Production proves each link: `"Voided in full at Clover; ledger mirrored"`, `"Partial reversal of 4714 cents"`, `"Already reconciled: 6250 cents reversed, 6250 recorded"`, `"Settled by the sweep: …"`.

What was wrong was the arithmetic underneath it, in three places, none of which any gate could see.

**1. A second partial refund could be lost forever.** The reversal row was identified by `refunds.elements[0].id` — the FIRST refund, whatever the reversal being recorded actually was. `payments_processor_identity` is unique on `(processor, processor_payment_id)`, so a second partial refund collided with the first and the insert was **refused**: two `"duplicate key value violates unique constraint"` rows sit in `payment_webhook_events` from 8 August. Whether it healed was luck — a later refund could shift `elements[0]` and let the next attempt through; with no later refund the money was never recorded and the booking overstated what it held, permanently. Now **one row per reversal, keyed to its own Clover id**, which turns the unique index from a hazard into the idempotency guarantee: a replay is refused on the right row, and `23505` is read as "already recorded". **The gap survives as a ceiling and must not be removed** — payments reversed before this change carry one aggregate row naming no individual refund, so without the cap every refund id looks unrecorded and the money is written a second time, on an append-only table.

**2. A listed refund was treated as proof money moved.** `refunds.elements` was summed whatever each element said. The payloads carry `status` and `voided` — verified against a stored one, `{"id":"HNJGHGMPZ55BA","amount":40,"status":"SUCCESS","voided":false,…}` — and neither was in the type. This is the same trap `payment.result === "SUCCESS"` exists to stop, one level down and never applied there: a FAILED or since-voided refund counted as money given back, so the ledger records a reversal that never happened and the facility is out of pocket. A **missing** status still counts as SUCCESS, deliberately — every payload measured has the field, and refusing a real refund because an API version omitted it is the error that leaves the customer out of pocket instead.

**3. A partial reversal moved the balance by the wrong amount.** It was written as `subtotal: -gap, tax: 0, tip: 0`, described in the outcome as a labelling caveat. It was not: `private.booking_amount_paid` is `sum(grand_total - tip - tax)`, so everything charged to subtotal moves what the customer is shown as owing. **Refunding a $20 tip in Clover dropped `amount_paid` by $20 and flipped a fully-paid booking into owing $20 nobody owed.** Clover does not say how a refund splits — a refund element carries no tax or tip of its own, and the nested `payment` holds the ORIGINAL's — so the split is now derived in proportion to the payment being reversed. Not certainly right; bounded instead of maximal in exactly the case that used to be worst.

**And the retry loop never gave up.** Three deliveries sat `failed` from 8 to 26 August, retried every fifteen minutes — roughly 1,700 attempts — with no possibility of succeeding: they name merchant `796PJWMTZZH01` and the facility's live connection is `5ZQH512PQ0KP1`. `reconcilePayment` reads with the CURRENT merchant's token, Clover cannot answer for another estate, that returns `unreadable`, and `unreadable` was skipped and left for next time. `drainUnsettled` now takes the merchant and closes a delivery from a superseded one without a Clover read at all.

**Why none of this was caught:** the arithmetic lived inside `reconcile.ts`, which imports `server-only` and cannot be loaded outside a server request, so nothing could assert it. It is now `lib/clover/reversal.ts` — pure, no imports — and `tests/e2e/clover-reversal.spec.ts` drives it in the push gate with no browser and no network. **It earned its place on the first run**, catching a bug in the new code: the zero-total guard was ordered after `amount >= grandTotal`, which is true for every amount when the total is 0, so a reversal came back with components summing to 0 and a `grand_total` of the full amount — a money row disagreeing with its own parts.

**Now visible, which it never was.** `last_swept_at` had been written since the sweep was built and read by nothing, so a reconciler that stopped a week ago looked identical to one that ran a minute ago. Yipyy Pay → Overview now shows "Refunds made in Clover may not be reaching Yipyy" when the sweep is stale or deliveries are unsettled — silent when healthy, because a permanently green badge is how people stop reading it. On the box, `OnFailure=yipyy-alert@%n.service` gives the sweep script's exit 2 ("completed WITH PROBLEMS") somewhere to go. **That handler logs; it does not page anybody**, and `deploy/yipyy-alert@.service` says so rather than pretending — wiring it to a real channel needs a credential and a decision about who is on call.

**Still open:** the proportional split is a derivation, not Clover's own answer, and a genuine tip-only refund is still spread across all three components. Clover does not expose the breakdown on a refund; closing it properly needs the order read back through `processor_order_id`, which is the same missing piece as item-level retail returns below.
...
**Where the thousand rows came from, measured the same day — and a wrong
diagnosis, corrected.** 1,114 of the e2e tenant's 1,459 bookings carry an
`[e2e …]` marker in `special_requests` and date back to **2026-08-04**:
`payment-ledger` 271, `dashboard-board` 215, `checkout-truth` 201,
`payment-screens` 178, `booking-form` 120, `gift-card-payment` 93, and a tail.

**This was first written up here as a six-week LEAK — specs failing to clean up
after themselves. That was wrong, and the correction matters more than the
original claim.** Measured immediately after: **1,108 of those rows are
`cancelled`, and 1,098 of them have `payments` rows attached.** The cleanup ran
and did exactly what it is supposed to do. `booking-payment-ledger`'s own
teardown says why it cancels rather than deletes:

> The payment row cannot be removed, so the only honest cleanup is the one a
> business would do.

So these are not residue. They are **cancellation tombstones on an append-only
ledger**, and they persist BY DESIGN. There is no leak to fix at source, and
deleting them would mean destroying append-only financial records — the one
thing the ledger exists to prevent. Do not "tidy" them.

**What is actually true:** running money specs against a shared database grows
the bookings table permanently, about a thousand rows per six weeks, and nothing
about that is a defect. The consequence was the 1000-row cap above, and that is
fixed by paging the read — correctly, because the row count was never going to
come back down.

**Do instead:** when a spec's subject cannot be deleted, say so in the teardown
the way that one does, and expect the table to grow. The thing to watch is any
UNBOUNDED read over it — `/api/bookings` already takes ~12 s at this size, and
