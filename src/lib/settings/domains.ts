...
import { bookingRulesSchema } from "@/types/facility";
...
/**
 * What the app assumes before a facility has said otherwise.
 *
 * Carried over from the fixture rather than reinvented, because these values
 * are already what every existing screen has been rendering — changing them in
 * the same commit that makes them real would be two changes wearing one hat.
 */
