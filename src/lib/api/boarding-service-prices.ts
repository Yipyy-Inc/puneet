...
/**
 * Replace one boarding service's branch prices.
 *
 ...
 * into the other fails the route type check at build time. The daycare twin of
 * this file exists for exactly the same reason.
 *
 * ── REPLACED, NOT MERGED ──────────────────────────────────────────────────
 *
 * A branch removed from the map must LOSE its override. A merge would leave it
 * charging a price the facility had just deleted.
 *
 ...
 */
export async function writeBoardingBranchPrices(
  ...
  const { error: deleteError } = await supabase
    .from("boarding_service_location_prices")
    .delete()
