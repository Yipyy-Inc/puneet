      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <ul className="mt-1.5 space-y-1.5">
            {alerts.map((alert) => {
              const meta =
                CATEGORY_META[alert.category] ?? CATEGORY_META.general;
              return (
                <li
                  key={alert.id}
                  className="flex items-start gap-1.5 text-[13px]/relaxed text-rose-900"
                >
                  <span
                    aria-hidden
                    className="mt-1.5 size-1.5 shrink-0 rounded-full bg-rose-700"
                  />
                  <span className="min-w-0">
                    <span
                      className={`mr-1.5 inline-flex items-center rounded-sm border px-1.5 align-[1px] text-[9.5px] font-bold tracking-wide uppercase ${meta.cls}`}
                    >
                      {meta.label}
                    </span>
                    {alert.note}
          ...
          <p className="mt-1.5 text-[10.5px] text-rose-700/80">
            Alerts stay active until a trainer or manager deactivates them from
            the Notes tab with a reason.
          </p>
        </div>
      </div>
    </section>
