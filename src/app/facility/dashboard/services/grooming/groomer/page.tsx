...
export default function GroomerDashboardPage() {
  ...
  const todayAppointments = useMemo(() => {
    ...
    return appointmentsData.filter(
      (apt) => apt.date === today && apt.stylistId === currentStylist.id,
    );
  }, [currentStylist, appointmentsData]);
  ...
  const handleMarkReady = (appointment: GroomingAppointment) => {
  ...
  const handleSavePhotos = () => {
    if (!selectedAppointment || !currentStylist) return;
