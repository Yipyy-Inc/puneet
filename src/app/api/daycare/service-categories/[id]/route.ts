...
// ============================================================================
// Rename a daycare category, or take it off the menu.
//
// ── WHY THESE EXIST ───────────────────────────────────────────────────────
//
...
//
// No migration was needed: `daycare_categories_write` is already `for all` to
// authenticated under `manage_services`, so UPDATE and DELETE were permitted by
// the database the whole time. Only the HTTP verbs were missing.
//
// ── DELETING A CATEGORY DOES NOT DELETE ITS SERVICES ──────────────────────
//
// `daycare_services.category_id` is `on delete set null`, so the services in a
...
export async function PATCH(
  ...
  const { data, error } = await supabase
    .from("daycare_service_categories")
    .update(patch)
    .eq("id", id)
