...
export const facilities = [
  ...
  },
  {
    ...
    // Grooming-specific operational gate. `requireBeforePhotos` is read by
    // the session panel when the appointment is In Progress to block the
    // "Mark Ready" action until staff have captured a before photo; the after
    // counterpart gates final completion. The form questions used to live on
    // a `groomingCheckinConfig` field alongside these flags — they have been
    // migrated into the unified per-service Express Check-in form (see
    // yipyygo-config.ts → formTemplates.grooming).
    groomingPhotoPolicy: {
