...
export function EstimatePdfDownload({
  ...
  // Shared by the facility's estimate drawer and the customer's pages, so the
  // words are a shell group and the document is in whoever prints it's
  // language — the same as the screen they printed it from.
  const t = useShellText("estimates");
  ...
  const handleDownload = useCallback(() => {
    ...
    const fmtDate = (d: string) => formatDateLong(d, locale);
...
<html lang="${locale}">
<head>
  <title>${fill("pdfTitle", { id: e.id, facility: p.businessName })}</title>
  <style> /* french-ok: CSS rules, not copy */
    ...
    .service-detail { font-size: 13px; color: #64748b; margin-top: 4px; } /* french-ok: CSS */
...
<body>
  <div class="header">
    ...
    <span class="estimate-tag">${t("pdfTag")}</span>
  </div>

  <div class="title">${fill("estimateFor", { pets: e.petNames.length > 0 ? formatList(e.petNames, locale) : (e.guestPetInfo?.name ?? t("yourPet")) })}</div>
  <div class="meta">
    ${fill("preparedFor", { name: e.clientName, date: fmtDate(e.createdAt.split("T")[0]) })}
    ${e.currentVersion ? ` · ${fill("revision", { n: e.currentVersion })}` : ""}
  </div>

  <div class="service-box">
    <div class="service-name">${serviceTypeLabel(locale, e.service)}${e.serviceType ? ` — ${e.serviceType}` : ""}</div>
    <div class="service-detail">
      ${e.endDate && e.endDate !== e.startDate ? fill("dateRange", { start: fmtDate(e.startDate), end: fmtDate(e.endDate) }) : fmtDate(e.startDate)}
      ${nights ? ` · ${fill(nights === 1 ? "nightOne" : "nightMany", { n: nights })}` : ""}
    </div>
    ${e.checkInTime ? `<div class="service-detail">${fill("checkInAt", { time: formatTimeOfDay(e.checkInTime, locale) })}${e.checkOutTime ? ` · ${fill("checkOutAt", { time: formatTimeOfDay(e.checkOutTime, locale) })}` : ""}</div>` : ""}
  </div>
  ...
  <div class="totals">
    <div class="total-row"><span>${t("subtotal")}</span><span>${money(e.subtotal)}</span></div>
    ${e.discount > 0 ? `<div class="total-row" style="color:#059669"><span>${t("discount")}${e.discountReason ? ` (${e.discountReason})` : ""}</span><span>${money(-e.discount)}</span></div>` : ""}
    <div class="total-row"><span>${fill("taxAt", { rate: formatPercent(e.taxRate * 100, locale) })}</span><span>${money(e.taxAmount)}</span></div>
    <div class="total-row grand"><span>${t("estimatedTotal")}</span><span>${money(e.total)}</span></div>
    ${e.depositRequired ? `<div class="total-row" style="color:#2563eb"><span>${t("depositRequired")}</span><span>${money(e.depositRequired)}</span></div>` : ""}
  </div>

  ${e.publicNote ? `<div class="note">"${e.publicNote}"</div>` : ""}

  ${e.expiresAt ? `<div style="margin-top:20px;font-size:12px;color:#94a3b8">${fill("expiresOn", { date: fmtDate(e.expiresAt.split("T")[0]) })}</div>` : ""}
