      ...
      {filteredCameras.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-14 text-center">
            <YipyyPose name="sleeping" size={132} float />
            <div>
              <p className="font-medium">
                {cameras.length === 0
                  ? "No cameras set up yet"
                  : "No cameras in that location"}
              </p>
              <p className="text-muted-foreground mt-1 max-w-sm text-sm">
                {cameras.length === 0
                  ? "Once a camera is added it appears here, with its own live view and kennel coverage."
                  : "Choose another location, or clear the filter to see every camera."}
              </p>
            </div>
          </CardContent>
        </Card>
