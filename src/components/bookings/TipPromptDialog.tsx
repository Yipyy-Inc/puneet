...
import { Heart, PawPrint, Sparkles, Users, X, ShieldCheck } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogTitle,
...
interface TipPromptDialogProps {
  ...
  petName?: string;
  serviceLabel?: string;
  /** Staff members who cared for the pet — drives personalization */
  staff?: Array<{ name: string; avatarUrl?: string; role?: string }>;
  /** Label for the confirmation action (wizard vs post-stay reminder) */
  confirmLabel?: string;
  /** Shown instead of the pet-voice line on post-stay flows */
  contextTitle?: string;
...
export function TipPromptDialog({
  ...
  // Re-seed when the dialog opens so the preferred amount is the default
  useEffect(() => {
    if (open) {
      setLocalTip(tipAmount > 0 ? tipAmount : preferredAmount);
      setShowCustom(false);
      setCustomValue("");
    }
  }, [open, tipAmount, preferredAmount]);
  ...
  const handleConfirm = () => {
    onTipChange(Math.round(localTip * 100) / 100);
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="from-primary/15 via-primary/5 relative bg-gradient-to-b to-transparent px-6 pt-7 pb-4 text-center">
          <div className="mb-3 flex justify-center">
            <div className="bg-primary/15 ring-primary/20 relative flex size-14 items-center justify-center rounded-full ring-4">
              ...
              <PawPrint className="bg-primary text-primary-foreground absolute -right-1 -bottom-1 size-6 rounded-full p-1" />
            </div>
          </div>
        ...
        <div className="space-y-4 px-6 pt-2 pb-5">
          ...
          {/* Tip options — 2×2 grid, preferred hero-styled */}
          ...
          {showRoundUp && !showCustom && (
            <button
              type="button"
              onClick={() => setLocalTip(roundUpAmount)}
