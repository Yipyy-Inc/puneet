...
import { Plus, X, Pill, Trash2, ChevronDown, ChevronUp, Clock, ShieldAlert } from "lucide-react";
...
const HIGH_RISK_KEYWORDS = ["insulin", "seizure", "phenobarbital", "prednisone", "thyroid", "heart"];
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        {medications.map((item, index) => {
          ...
          return (
            <Card key={item.id} className={cn(showHighRisk && "border-red-200")}>
              <CardContent className="pt-4 space-y-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-medium text-sm">{item.name || "New Medication"}</h4>
                        {showHighRisk && (
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.amount}{item.strength ? ` (${item.strength})` : ""} &middot; {MED_FREQUENCIES.find((f) => f.value === item.frequency)?.label || item.frequency}
                        {item.times.length > 0 && ` &middot; ${item.times.length} times`}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    <Button type="button" variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); remove(index); }} className="text-destructive hover:text-destructive text-xs">
                      Remove
                    </Button>
                ...
                {isExpanded && (
                  <div className="space-y-4">
                    ...
                    {selectedPets.length > 1 && (
                      <Select value={item.petId?.toString() ?? ""} onValueChange={(v) => updateMed(index, { petId: parseInt(v, 10) })}>
                        <SelectTrigger className="h-9"><SelectValue placeholder="Select pet" /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Input value={item.name} onChange={(e) => updateMed(index, { name: e.target.value })} placeholder="e.g., Apoquel" className="h-9" />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs font-medium">Purpose</Label>
                        <Input value={item.purpose || ""} onChange={(e) => updateMed(index, { purpose: e.target.value })} placeholder="e.g., Allergy" className="h-9" />
                      </div>
                    </div>
                    ...
                    <div className="bg-muted/30 rounded-lg p-3 space-y-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        <div className="space-y-1">
                          ...
                          <Input value={item.amount} onChange={(e) => updateMed(index, { amount: e.target.value })} placeholder="1 tablet" className="h-8 text-xs" />
                        </div>
                        <div className="space-y-1">
                          <Label className="text-[11px]">Strength</Label>
                          <Input value={item.strength || ""} onChange={(e) => updateMed(index, { strength: e.target.value })} placeholder="16mg" className="h-8 text-xs" />
                        </div>
                        <div className="col-span-2 space-y-1">
                          <Label className="text-[11px]">Form</Label>
                          <Select value={item.form} onValueChange={(v) => updateMed(index, { form: v as MedForm })}>
                            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                            <SelectContent>
                              {MED_FORMS.map((f) => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                      </div>
                    ...
                    {/* Schedule */}
                    <div className="bg-muted/30 rounded-lg p-3 space-y-3">
                      <h5 className="text-xs font-medium text-muted-foreground">Schedule</h5>
                      <div className="flex flex-wrap gap-1">
                        {MED_FREQUENCIES.map((freq) => (
                          <button
                            key={freq.value}
                            type="button"
                            onClick={() => updateMed(index, { frequency: freq.value })}
                            className={cn(
                      ...
                      {item.frequency === "prn" && (
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            ...
                            <Input type="number" min={1} value={item.prnMaxPerDay || ""} onChange={(e) => updateMed(index, { prnMaxPerDay: parseInt(e.target.value) || undefined })} className="h-7 text-xs" />
                          </div>
                          <div className="space-y-1">
                            <Label className="text-[11px]">Trigger</Label>
                            <Input value={item.prnTrigger || ""} onChange={(e) => updateMed(index, { prnTrigger: e.target.value })} placeholder="e.g., Itching" className="h-7 text-xs" />
                          </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          <Input
                            ...
                            onBlur={(e) => { if (e.target.value) { addTime(index, e.target.value); e.target.value = ""; } }}
                          />
                        </div>
                        {item.times.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {item.times.map((t, ti) => (
                              <span key={ti} className="inline-flex items-center gap-0.5 rounded-full bg-violet-100 text-violet-700 px-2 py-0.5 text-[11px]">
                                <Clock className="h-2.5 w-2.5" />{formatTime(t)}
                                <button type="button" onClick={() => removeTime(index, ti)}><X className="h-2.5 w-2.5" /></button>
                              </span>
                            ))}
                          </div>
                        )}
                    ...
                    <div className="bg-muted/30 rounded-lg p-3 space-y-2">
                      <h5 className="text-xs font-medium text-muted-foreground">Administration</h5>
                      <div className="flex flex-wrap gap-1">
                        {ADMIN_INSTRUCTIONS.map((inst) => {
                          const active = (item.adminInstructions || []).includes(inst.value);
                          return (
                            <button
                              key={inst.value}
                              type="button"
                              ...
                              className={cn(
                                "rounded-full border px-2.5 py-0.5 text-[11px] font-medium transition-colors",
                                active ? "border-teal-300 bg-teal-50 text-teal-700" : "border-input hover:bg-muted/50"
                              )}
                            >
                              {inst.label}
                            </button>
                    ...
                    {/* If missed */}
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium">If Dose Missed</Label>
                      <div className="flex flex-wrap gap-1">
                        {MISSED_DOSE_OPTIONS.map((opt) => (
                          <button
                            key={opt.value}
                            type="button"
                            onClick={() => updateMed(index, { ifMissed: opt.value })}
                            className={cn(
                              "rounded-full border px-2.5 py-0.5 text-[11px] font-medium transition-colors",
                    ...
                    <div className="flex items-center justify-between rounded-md border p-2.5">
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">Extra caution required</p>
                      </div>
                      <Switch
                        checked={item.isHighRisk || false}
                        onCheckedChange={(checked) => updateMed(index, { isHighRisk: checked })}
                      />
                    </div>
