...
const RULE_TYPES: RuleConfig[] = [
  { value: "max_weight",     label: "Max Weight",          valueType: "number", unit: "lbs", placeholder: "e.g. 60"  },
  { value: "min_weight",     label: "Min Weight",          valueType: "number", unit: "lbs", placeholder: "e.g. 40"  },
  { value: "max_pets",       label: "Max Pets per Booking", valueType: "number", placeholder: "e.g. 2" },
  { value: "single_pet_only", label: "Single Pet Only",    valueType: "none"  },
  {
    value: "pet_type", label: "Pet Type Restriction", valueType: "select",
    options: [
  ...
  {
    value: "size_restriction", label: "Size Group", valueType: "select",
    options: [
...
function blankRule(type: RoomRuleType): RoomRule {
  const defaults: Record<RoomRuleType, number | string> = {
    max_weight: 60, min_weight: 40, max_pets: 2,
    single_pet_only: 1, pet_type: "dog", size_restriction: "medium",
  };
  return { id: `rule-${Date.now()}`, type, value: defaults[type], clientMessage: "", enabled: true };
}
...
export function CategoryFormDialog({ open, editing, facilityId = 11, onClose, onSave }: Props) {
  ...
  const addRule = () => {
    if (!addType) return;
    setForm((p) => ({ ...p, rules: [...p.rules, blankRule(addType as RoomRuleType)] }));
    setAddType("");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="max-w-lg max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg">{editing ? "Edit Room Category" : "Create Room Category"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5 py-1">
          ...
          <div className="space-y-4">
            ...
            <div className={editing ? "grid grid-cols-2 gap-4" : "grid grid-cols-3 gap-4"}>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, defaultCapacity: Math.max(1, parseInt(e.target.value) || 1) })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, defaultBasePrice: parseFloat(e.target.value) || undefined })}
                  placeholder="55"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <Switch checked={form.visibleToClients} onCheckedChange={(v) => setForm({ ...form, visibleToClients: v })} />
              <Label className="cursor-pointer font-normal text-sm">Visible to clients in the booking flow</Label>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              <p className="font-semibold text-sm">Booking Rules</p>
              <p className="text-muted-foreground text-xs mt-0.5 flex items-start gap-1">
                <Info className="size-3.5 mt-0.5 shrink-0" />
                Rules are enforced during client booking. When a rule is triggered, the custom message is shown to the client and the room is greyed out.
              </p>
            </div>
            ...
            {form.rules.length > 0 && (
              <div className="space-y-2">
                {form.rules.map((rule) => {
                  ...
                  return <RuleRow key={rule.id} rule={rule} config={cfg} onPatch={(p) => patchRule(rule.id, p)} onRemove={() => removeRule(rule.id)} />;
                })}
              </div>
            )}

            <div className="flex gap-2">
              <Select value={addType} onValueChange={(v) => setAddType(v as RoomRuleType)}>
              ...
              <Button size="sm" variant="outline" disabled={!addType} onClick={addRule} className="h-8 px-3 shrink-0">
    ...
    <div className="rounded-lg border bg-muted/20 p-3 space-y-2.5">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <Switch checked={rule.enabled} onCheckedChange={(v) => onPatch({ enabled: v })} className="scale-75" />
      ...
      <div className="space-y-1">
        <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">Client message when blocked</p>
        <Input
