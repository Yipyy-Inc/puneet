...
| `bun run test:e2e`                    | The whole Playwright suite (57 files, ~45 min, one worker — see the debt map before trusting a run) |
| `bun run test:e2e:ci`                 | The 25 specs CI runs on every PR — auth & access, daily operations, scheduling; ~15 min             |
