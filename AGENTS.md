...
There **is** a test runner: Playwright, 115 spec files under [tests/e2e/](tests/e2e/), driving a real browser against a real server. It was described here as absent long after it existed. "Green" = the CI gates, the auth & access specs, and a look at the touched journey.
...
| `bun run check:settings-french`        | Fails if a settings section a French user reads in English regresses — the static form of the two-locale render diff. 49 of 50 baselined |
