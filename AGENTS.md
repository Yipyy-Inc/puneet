...
There **is** a test runner: Playwright, 82 spec files under [tests/e2e/](tests/e2e/), driving a real browser against a real server. It was described here as absent long after it existed. "Green" = the CI gates, the auth & access specs, and a look at the touched journey.
...
| `bun run test:e2e`                     | The whole Playwright suite (82 files, ~45 min, one worker — see the debt map before trusting a run)                   |
| `bun run test:e2e:ci`                  | The 50 specs CI runs on every PR — auth & access, daily operations, scheduling, payroll, loyalty, report cards, tasks |
| `bun run test:sql`                     | The 44 SQL files — RLS, grants, database invariants. Runs in CI. ~90s; needs `SUPABASE_DB_URL`                        |
