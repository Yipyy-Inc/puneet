...
standing rule. **Stages 0–11 are done** (documents and assets, the `@theme` tokens, the empty-state
...
1. **Ground** — Read the relevant doc below and the neighboring code before writing anything. **Touching an interface? Read [docs/design-system/design-system.md](docs/design-system/design-system.md) and open the reference page `docs/design-system/Yipyy Design System.dc.html` in a browser first** — where the prose and the page disagree, the page is right. Inventory what exists (components, hooks, `src/lib/api/` factories, `src/data/` shapes) and reuse it — never recreate. **Confirm the target component is actually wired in before editing it** — a file that typechecks can still be dead code (a superseded duplicate). When a task names a component (especially a settings section), find what actually renders it first; if nothing does, it is dead and the live screen is elsewhere (e.g. the old `RolesPermissionsSettings.tsx` vs the live `FacilityRolesStudio.tsx`, 2026-07). **Settings is a route per section now** — `src/app/facility/dashboard/settings/<segment>/page.tsx`, with the section body in `_sections/<segment>.tsx` and its cards in `_components/`. The registry `src/lib/settings/nav.ts` is the single list the rail, the index, the redirects, the permission guard and `bun run check:settings-routes` all read; a section that is not in it does not exist. It replaced one 4,700-line `settings/page.tsx` that switched on `?section=`, so any instruction to grep that file is stale. The gate `bun run check:settings-wiring` fails on any orphaned `*Settings.tsx`; `bun run prune` (Knip) also flags files imported nowhere.
...
There **is** a test runner: Playwright, 119 spec files under [tests/e2e/](tests/e2e/), driving a real browser against a real server. It was described here as absent long after it existed. "Green" = the CI gates, the auth & access specs, and a look at the touched journey.
...
| `bun run test:e2e`                     | The whole Playwright suite (119 files, ~45 min, one worker — see the debt map before trusting a run)                                                                                                                                                                                                                                                                                                                             |
| `bun run test:e2e:gate`                | The 29 specs CI runs on every push — the authorisation boundary, and money                                                                                                                                                                                                                                                                                                                                                       |
...
| `bun run test:sql`                     | The 80 SQL files — RLS, grants, database invariants. Runs in CI. ~90s; needs `SUPABASE_DB_URL`                                                                                                                                                                                                                                                                                                                                   |
...
| `bun run check:settings-persistence`   | Fails if a settings section's save path reaches nothing that can leave the browser — the WRITE side of the row above. Ratcheted at 3                                                                                                                                                                                                                                                                                             |
| `bun run check:ui-french`              | Fails if a surface a French user reads in English regresses. Settings (**all 50 done**), the root + four portal shells and the shadcn primitives are **at zero at their walk depth** — the shells are measured 2 hops from their layout, and at 3 they hold 524 strings, so read "at zero" as a claim about the depth (debt map, 2026-09-09). `staff` walks 5 hops and is ratcheted per file: 725 strings on 2026-09-08, 204 now |
...
| `bun run check:control-heights`        | Fails if a call site writes a fixed height under 40px onto a primitive that already ships `min-h-10 max-lg:min-h-12` — §1 and §6 rule 7. Ratcheted PER FILE at 676 across 253 files; settings is at zero                                                                                                                                                                                                                         |
| `bun run check:nav-icons`              | Fails if a nav glyph drifts from `icon-map.json`, or if any glyph lands on two areas — §5b1                                                                                                                                                                                                                                                                                                                                      |
| `bun run check:hardcoded-locale`       | Fails if a formatter is told a locale the user did not choose — §5q. TWO ratchets: 526 told a literal tag, 432 told NOTHING — `toLocaleDateString()`, `toLocaleTimeString([], …)` and `toLocaleString(undefined, …)` all take the machine's locale (added 2026-09-09)                                                                                                                                                            |
| `bun run check:frozen-translator`      | Fails if a `useMemo` or `useCallback` calls a translator without depending on it — the memo serves the pre-hydration English forever. **At zero**                                                                                                                                                                                                                                                                                |
...
**The green sequence (run before claiming done):** `bun run typecheck && bun run lint && bun run format:check && bun run test:unit && bun run check:all`, then for UI changes `bun run dev` and visually confirm the touched [critical user journey](docs/product/critical-user-journeys.md). Run `bun run build` for anything structural (routing, layouts, server/client boundaries). Use **bun** only — never npm/yarn/pnpm.

**`check:all` was added on 2026-09-09, and the sequence above did not have it before.** There are 34 `check:*` scripts and no way to run them all, so "run the checks" meant "run the ones you remember" — and a push went red on `check:success-claims` after a local sweep of six came back green. Converting a `toast.success("Task added")` into `toast.success(t("taskAdded"))` dropped a baselined claim; the ratchet correctly demanded the entry be removed; nothing local had asked. Every ratchet here works the same way — **an unrecorded win leaks back** — which only holds if the whole set is one command. It discovers the scripts from `package.json` rather than listing them, so a check added tomorrow runs today, and it collects failures instead of stopping at the first.
...
So the gate is **29** specs, and it runs on every push: the authorisation boundary (`admin-portal-enforced`, `facility-access-level`, `facility-identity`, `employee-identity`, `server-permissions`, `staff-field-exposure`, `call-qa-exposure`, `assigned-scope`, `role-editor-writes`, `staff-invite-gate`, `passkey-auth`, `automation-send-boundary`, `review-survey-token`, `customer-setting-reads`, `tag-catalogue`, `twilio-webhook-signature`) and money (`yipyy-pay`, `gift-cards`, `retail-charge`, `clover-reversal`, `clover-tips`, `payment-channel`, `saved-cards`, `clover-capabilities`, `clover-device`), plus `booking-write-integrity`, which is where a production 500 was once found. And the full suite is **88** specs, running nightly at 03:00 UTC, on `workflow_dispatch`, and whenever you run it yourself. The coverage is not dropped — it is rescheduled.
...
- **Push straight to `main`; do not open a PR** unless asked. Decided
  ...
  **How a change reaches production now:**
  1. Push to `main`. CI runs the 29-spec gate, the SQL tests and the checks.
