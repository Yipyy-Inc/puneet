...
| `bun run test:e2e`                    | The whole Playwright suite (55 files, ~45 min, one worker — see the debt map before trusting a run) |
| `bun run test:e2e:ci`                 | The 23 specs CI runs on every PR — auth & access, daily operations, scheduling; ~14 min             |
