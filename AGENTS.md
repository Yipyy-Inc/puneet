...
| `bun run test:e2e`                    | The whole Playwright suite (61 files, ~45 min, one worker — see the debt map before trusting a run) |
| `bun run test:e2e:ci`                 | The 29 specs CI runs on every PR — auth & access, daily operations, scheduling, payroll; ~18 min    |
