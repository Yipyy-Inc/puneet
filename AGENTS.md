...
| `bun run test:e2e`                    | The whole Playwright suite (60 files, ~45 min, one worker — see the debt map before trusting a run) |
| `bun run test:e2e:ci`                 | The 28 specs CI runs on every PR — auth & access, daily operations, scheduling; ~17 min             |
