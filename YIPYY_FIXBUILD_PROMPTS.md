...
Round 2. These are the 19 changes from the client's *Fix & Build Specification* (changes only — working items omitted). Paste into Claude Code **in order**: 5 CRITICAL first (FB-1…FB-5), then 14 HIGH (FB-6…FB-19). Several tasks modify work from the original 84-prompt build — cross-references are noted so the agent edits existing components instead of creating duplicates.
...
*19 tasks. Build the 5 CRITICAL (FB-1…FB-5) before the 14 HIGH (FB-6…FB-19). FB-5's 12-step wizard references the separate "Custom Module Integration" document — have that open when you run it. Paste one prompt at a time; let the agent plan, implement, typecheck, and lint before moving on.*
