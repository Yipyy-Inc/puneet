...
async function categoryList(page: Page): Promise<Category[]> {
  ...
  const body: unknown = await res.json();
  // A cast is a claim: a 500 answers `{error}`, and `.filter` on that throws
  // INSIDE the teardown, where it takes the whole cleanup with it.
...
async function sweep(page: Page): Promise<number> {
  ...
  // They are swept at all only because DELETE exists as of 2026-09-24 — before
  // that a category written by a test was permanent.
...
test.afterAll(async ({ browser }) => {
  ...
  try {
    ...
    console.log(
      `cleanup: ${await sweep(page)} service(s)/category(ies) removed, ` +
...
test.describe("the daycare menu", () => {
  ...
  test("a category is created, renamed and removed — and its services survive", async ({
    ...
    // THE WRITE PATH NOBODY HAD RUN. The collection route shipped GET and POST
    // on 2026-09-24 and the screen that called them arrived the same day, so
    // until this test the only proof a category could be written was that the
    // code compiled. PATCH and DELETE followed because a create button on its
    // own makes every typo permanent.
    await signIn(page, ACCOUNTS.owner);

    const made = await page.request.post(CATEGORIES, {
      data: { name: `${MARKER} Puppy programmes` },
      failOnStatusCode: false,
    });
    expect(made.status(), await made.text()).toBe(201);
    const category = (await made.json()) as Category;
    expect(category.id, "the row comes back, not just an ok").toBeTruthy();
    expect(category.name).toBe(`${MARKER} Puppy programmes`);
